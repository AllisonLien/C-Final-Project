...
namespace Pet_Adoption_and_Shelter_Management_System
{
    public partial class FormPet : Form
    {
        ...
        public FormPet(string user, string role)
        {
            InitializeComponent();
            currentUser = user;
            currentRole = role;
        ...
        {
            FormHome home = new FormHome(currentUser, currentRole);
            home.Show();
            this.Hide();
        }
        ...
        private void LoadPets()
        {
            using (MySqlConnection conn = MYDB.Connect())
            {
                try
                {
                    ...
                    string sql = @"SELECT idpets AS 'ID',petname AS 'Name', species AS 'Species', breed AS 'Breed', age AS 'Age', gender AS 'Gender', location AS 'Location', decription AS 'Description' FROM pets";
